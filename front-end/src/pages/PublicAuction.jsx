    ...
    const loadAuction = async () => {
      try {
        ...
        if (auctionData.status === 'closed') {
          ...
          const endDate = new Date(auctionData.end_time);
          if (endDate < new Date()) {
            setAuctionEnded(true);
          }
        }
    ...
    return (
      <>
        <ItemDetailView
          ...
          onAuctionExpired={() => setAuctionEnded(true)}
        />
        <BidderModal
    ...
    return (
      <div className="min-h-screen bg-background">
        <div className="bg-red-50 border-b border-red-200 px-4 py-3">
          <div className="max-w-6xl mx-auto flex items-center justify-center gap-2 text-red-800">
            <AlertCircle className="w-5 h-5" />
            <span className="font-medium">This auction has ended</span>
          </div>
        </div>

        <div className="max-w-6xl mx-auto px-4 py-8">
          <div className="mb-8 text-center">
            <Badge variant="outline" className="mb-2 bg-red-100 text-red-700 border-red-300">Closed</Badge>
            <h1 className="text-3xl font-bold text-foreground">{auction.auction_name || 'Untitled Auction'}</h1>
            {auction.description && (
          ...
          <h2 className="text-lg font-semibold mb-4">Final Results - {items.length} Item{items.length !== 1 ? 's' : ''}</h2>
